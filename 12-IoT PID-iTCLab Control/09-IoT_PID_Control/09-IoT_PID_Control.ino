...
const char* ssid = "Wizsth"; // Enter your WiFi name
const char* password =  "601310663124"; // Enter WiFi password
...
//    if (client.connect("ESP32Client", mqttUser, mqttPassword )) {
    ...
    client.subscribe("autoSetRai");
    client.subscribe("SetPointRai");    
    client.subscribe("Nilai_KcRai");
    client.subscribe("Nilai_tauIRai");
    client.subscribe("Nilai_tauDRai");
  }
}
...
void loop() { 
  ...
  if(Topic=="autoSetRai"){
   ...
   if(Topic=="Nilai_KcRai"){
    Kc=Payload.toFloat();
  }
   if(Topic=="Nilai_tauIRai"){
    tauI=Payload.toFloat();
  }
   if(Topic=="Nilai_tauDRai"){
    tauD=Payload.toFloat()/6;
  }
   if(Topic=="SetPointRai"){
    sp=Payload.toFloat();
  }
  Serial.println("<-------------------------->");
  ...
  client.publish("Suhu1Rai",suhu1);

  dtostrf(cel1, 1, 0, suhu2);
  client.publish("Suhu2Rai",suhu2);

  dtostrf(sp, 1, 0, Tampil_SP);
  client.publish("Tampil_SPRai",Tampil_SP);

  dtostrf(Kc, 1, 0, Tampil_Kc);
  client.publish("Tampil_KcRai",Tampil_Kc);

  dtostrf(tauI, 1, 0, Tampil_tauI);
  client.publish("Tampil_tauIRai",Tampil_tauI);

  dtostrf(tauD, 1, 0, Tampil_tauD);
  client.publish("Tampil_tauDRai",Tampil_tauD);  
  ...
  if(autoSet==1){
    ...
    // bisa diubah2, atau sesuai yg muncul terakhir
     ...
     client.publish("Nilai_opRai",Nilai_op);
  }
}
